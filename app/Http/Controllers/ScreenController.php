...
public function getScreensWithReviewsByRole(Request $request)
{
    ...
    $screens = Screen::whereHas('reviews', function ($query) use ($isDeveloper, $reviewType) {
        ...
        ->whereHas('task.assignments', function ($q) use ($user) {
            $q->where('employee_id', $user->id);
        })
        ->with([
            'task:id,label',
            'requestedApis.implementedApis',
            'reviews' => function ($query) use ($isDeveloper, $reviewType) {
                $query->where('is_resolved', false);
        ...
        ->map(function ($screen) {
            return [
                ...
                'integrated'  => $screen->integrated,
                'backend_approved' => $screen->requestedApis
                    ->flatMap(fn($reqApi) => $reqApi->implementedApis)
                    ->where('status', 'tested')
                    ->isNotEmpty(),
...
public function getScreenWithReviewsByType(Request $request, $id)
{
    ...
    $validTypes = ['backend', 'frontend', 'ui', 'mobile'];
